...
export async function createHarvest(formData: FormData): Promise<ActionResult<Harvest>> {
  ...
  const { data, error } = await supabase.rpc('create_harvest_with_stock', {
    ...
    p_created_by: userId,
    p_row_id: parsed.data.row_id ?? undefined,
    p_lieu_sauvage: parsed.data.lieu_sauvage ?? undefined,
    p_temps_min: parsed.data.temps_min ?? undefined,
    p_commentaire: parsed.data.commentaire ?? undefined,
  })
