...
export default function AchatsClient({ purchases: initialPurchases, varieties, stockLevels, certifBio = false, actions }: Props) {
  ...
  const displayed = purchases.filter(item => {
    ...
    return (
      ...
      (item.fournisseur && normalize(item.fournisseur).includes(q)) ||
  ...
  function openEdit(item: StockPurchaseWithVariety) {
    setEditingItem(item)
    setSlideOverOpen(true)
  }
    ...
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: '#9CA89D' }}>
            {purchases.length} enregistrement{purchases.length !== 1 ? 's' : ''}
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            type="text"
            placeholder="Rechercher par variete, fournisseur…"
            value={search}
              ...
              {displayed.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3" style={{ color: '#2C3E2D' }}>
                      {item.fournisseur ?? <Dash />}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color: '#6B7B6C' }}>
                      {item.prix != null ? `${item.prix.toFixed(2)} €` : <Dash />}
                    </td>
                    <td className="px-4 py-3">
                      {item.certif_ab && (
                        <span
