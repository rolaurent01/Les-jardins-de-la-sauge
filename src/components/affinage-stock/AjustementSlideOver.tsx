...
type Props = {
  ...
  item: StockAdjustmentWithVariety | null
...
export default function AjustementSlideOver({
  ...
  // Stock disponible pour sortie
  const stockDispo = useMemo(() => {
    if (!selectedVarietyId || !selectedPartie || !selectedEtat) return null
    const match = stockLevels.find(
      s => s.variety_id === selectedVarietyId
        && s.partie_plante === selectedPartie
        && s.etat_plante === selectedEtat,
    )
    return match?.stock_g ?? 0
  }, [selectedVarietyId, selectedPartie, selectedEtat, stockLevels])

  const poidsNum = parseFloat(poidsValue) || 0
  const stockInsuffisant = type === 'sortie' && stockDispo !== null && poidsNum > stockDispo

  const titleLabel = isEdit ? 'Modifier l\'ajustement' : 'Nouvel ajustement'
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto flex flex-col">
          <div className="px-6 py-5 space-y-5 flex-1">
            ...
            {/* Toggle type */}
            {!isEdit && (
              <Field label="Type d'ajustement" required>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => onTypeChange('entree')}
                    className="flex-1 py-2 rounded-lg text-sm font-medium transition-colors"
                    style={{
                      backgroundColor: type === 'entree' ? 'var(--color-primary)' : 'transparent',
                      color: type === 'entree' ? '#F9F8F6' : '#6B7B6C',
                      border: type === 'entree' ? '1px solid var(--color-primary)' : '1px solid #D8E0D9',
                    }}
                  >
                    🟢 Entree
                  </button>
                  <button
                    type="button"
                    onClick={() => onTypeChange('sortie')}
                    className="flex-1 py-2 rounded-lg text-sm font-medium transition-colors"
                    style={{
                      backgroundColor: type === 'sortie' ? '#DDA15E' : 'transparent',
                      color: type === 'sortie' ? '#F9F8F6' : '#6B7B6C',
                      border: type === 'sortie' ? '1px solid #DDA15E' : '1px solid #D8E0D9',
                    }}
                  >
                    🔴 Sortie
                  </button>
            ...
            <Field label="Variete" required>
              <div className="flex items-center gap-2 mb-1.5">
                <QuickAddVariety
                  ...
                  onCreated={(newVar) => {
                    setAllVarieties(prev => [...prev, { id: newVar.id, nom_vernaculaire: newVar.nom_vernaculaire, parties_utilisees: newVar.parties_utilisees }])
            ...
            <Field label="Etat de la plante" required>
              ...
              >
                ...
                {ETATS.map(etat => (
                  <option key={etat} value={etat}>{ETAT_PLANTE_LABELS[etat] ?? etat}</option>
                ))}
              </select>
            </Field>

            {/* Info stock (visible en mode sortie) */}
            {type === 'sortie' && stockDispo !== null && (
              <div
                className="text-sm px-3 py-2 rounded-lg"
                style={{
                  backgroundColor: stockInsuffisant ? '#FEF3C7' : '#F0FFF4',
                  color: stockInsuffisant ? '#92400E' : '#166534',
                  border: `1px solid ${stockInsuffisant ? '#DDA15E44' : '#16653444'}`,
                }}
              >
                {stockInsuffisant
                  ? `⚠️ Stock insuffisant (${formatWeight(stockDispo)} disponibles)`
                  : `Stock disponible : ${formatWeight(stockDispo)}`}
              </div>
            )}
